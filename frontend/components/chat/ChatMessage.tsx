...
function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    ...
    >
      <div
        className={`max-w-full sm:max-w-[85%] md:max-w-[75%] lg:max-w-[70%] rounded-xl px-4 py-3 ${
          isUser
            ? "bg-gradient-to-r from-yellow-500 to-yellow-600 text-[#1a1410] shadow-lg"
            : "glass-card text-yellow-100 border border-yellow-500/20"
        }`}
      >
        {/* Message Content */}
        ...
        {message.created_at && (
          <div
            className={`text-xs mt-2 ${
              isUser ? "text-[#1a1410]/70" : "text-yellow-100/60"
            }`}
          >
